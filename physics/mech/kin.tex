...
One way to compute it is to break it down to its components, \[
    \dv{\vec{r}}{t} = \dv{x}{t}  \unitv{i} + \dv{y}{t}  \unitv{j} + \dv{z}{t}  \unitv{k}
.\]
But since the components of the vector depend on the reference frame, we may generalise the differentiation of a vector
in two parts.}

\begin{casework}
        \ii \(\displaystyle\dv{\vec{A}}{t} \perp \vec{A}\)
        \ii \(\displaystyle\dv{\vec{A}}{t} \parallel \vec{A}\)
\end{casework}

\para{For the first case, let us first consider a geometric argument for \(\increment \vec{A}\)
which is perpendicular to \(\vec{A}\). For sufficiently small values of \(\increment \vec{A}\),
...
where \(\theta\) is the angle between \(\vec{A}_1\) and \(\vec{A}_2\).}
