...
\sidenote{A little neat fact to note is that the trajectory is a double 
differentiable function of time. While this maybe apparent from the existence 
of forces, we may also write it out as a law, if we wish to, of course.}
...
This is called the \vocab{trajectory} of the particle. So when we solve the 
...
is a solution of an \vocab{equation of motion}, which is a second order differential 
...
To describe the motion of a particle, we some \vocab{degrees of freedom}.
...
An \vocab{instantaneous configuration} of some particles is any possible arrangement of 
those particles in space. The set of these configurations, \(\mathcal{Q}\) is called the \vocab{configuration space}.
...
\begin{marginfigure}
    \scalebox{2.3}{\incfig{tension,rope}}
