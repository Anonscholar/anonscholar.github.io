...
\begin{definition}
    ...
    \emph{Addition} on \(V\) is a function \(+ : V^2 \to V\) where \(\vect{v} + \vect{w} \in V\) for 
    any \(\vect{v}, \vect{w} \in V\).

    \noindent\emph{Scalar multiplication} on \(V\) is a function \(\cdot : V \times R \to V\) where 
    \(\lambda \vect{v} \in V\) for any \(\vect{v} \in V\) and \(\lambda \in \RR\). 
\end{definition}
...
\begin{definition}
    ...
    \begin{axioms}
        ...
        \item Associativity, \((u + w) + v = u + (w + v)\) and \(a(bv) = (ab)v\).
        \item Additive Identity, there an element \(0 \in V\) such that \(v + 0 = v\).
        \item Additive Inverse, there exist an element \(-v \in V\) for each \(v\) such that 
        \(v + (-v) = 0\).
        \item Multiplicative Identity, \(1v = v\)
        \item Distributive Property, \(a(u + v) = au + av\) and \(v(a + b) = va + vb\). 
    \end{axioms}
