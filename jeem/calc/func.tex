...
Range of \(R\) is the set of all second elements of the ordered pairs in the relation, that
are in set \(B\). \[Range_R = \{b \mid (a,b) \in R\}\]

\subsubsection{Co-domain}
\index{Relation!co-domain}

Co-domain of \(R \subseteq A \times B\) is the set \(B\) itself\footnote{You may note that \(Range_R \subseteq Co-domain_R\)}. 
...
\begin{definition}
    ...
    A function from \(A\) to \(B\) is a set of ordered pairs, \[
        f \subseteq \left\{(a,b) \mid a \in A, b \in B \land (x, y) \in f, (x, z) \in f \iff y = z \right\}
    \]
\end{definition}
...
If this is not the case, any image may have more than one pre-image under \(f\),
...
this leads us to motivate the existence of an \chvocab{Function}{inverse}
