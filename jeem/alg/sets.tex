...
\sidenote{These definitions are superfluous, we have not and will not define what
...
\sidenote{The symbol \(\in\) in \(x \in S\) is used to show that \(x\) is an element
...
\sidenote{ The set \(\{\varnothing\}\),is not a null set! In-fact it is the set that contains
...
\subsection{Subsets}
\sidenote{The empty set \(\varnothing\) is a subset of all sets.}
...
\sidenote{ Note that \(A\) is not a proper subset of \(A\).}
...
\sidenote{ Refer to \Cref{app: logic} for the symbol \(\iff\).}
...
\sidenote{ Because of the definition of powerset, \(A \subseteq B \iff P(A) \subseteq
P(B)\).}
...
\sidenote{`Or' in mathematics refer to something along the lines of one, the other, or both.
