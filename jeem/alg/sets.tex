...
\marginnote{ These definitions are superfluous, we have not and will not define what
...
\marginnote{The symbol \(\in\) in \(x \in S\) is used to show that \(x\) is an element
...
\marginnote{ The set \(\{\varnothing\}\),is not a null set! In-fact it is the set that contains
...
\subsection{Subsets}
\marginnote{The empty set \(\varnothing\) is a subset of all sets.}
...
\marginnote{ Note that \(A\) is not a proper subset of \(A\).}
...
\marginref{ Refer to \Cref{app: logic} for the symbol \(\iff\).}
...
\marginnote{ Because of the definition of powerset, \(A \subseteq B \iff P(A) \subseteq
P(B)\).}
...
\marginnote{`Or' in mathematics refer to something along the lines of one, the other, or both.
...
\[ A \times B = \Set{(x,y) \given x \in A, y \in B}\]

The use of cartesian product is mainly for ordered pairs. The \(n\)th cartesian product
of a set is represented as,
