...
\marginnote{An inequality or in-equation of \(x\) describes a property of \(x\) such
that the answer to it is generally an interval or a set of possible values which might satisfy it.
In contrast to equations, it rarely gives discrete answers, and instead of equality,
we have the ordering (\(<, >, \ge, \le\)) relations.}
...
\subsection{Inequalities concerning inverses}

If \(x \in \left(a, b\right)\), then, 

\textbf{Case I} \(ab > 0\)

If \(a\) and \(b\) have the same sign, then,
\[
    \frac{1}{x} \in \left(\frac{1}{b}, \frac{1}{a}\right)
\]
...
\[
    \frac{1}{x} \in \left(-\infty, \frac{1}{b}\right) \union \left(\frac{1}{a}, \infty\right)
\]
